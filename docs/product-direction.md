...
```text
控制 Obsidian Vault（非私密，安装插件）
  └── Bridge：菜单、状态、自动锁定、无密码配置
          │
          ├── Cryptomator Desktop：仅用于创建新的密文 Vault
          └── Cryptomator CLI + WinFsp：解锁、挂载、停止挂载
                    │
控制 Vault 的 Nutstore WebDAV 同步范围 ── 密文 Cryptomator Vault
                    │
控制 Vault 外的本机非同步目录 ── 明文挂载点 ── 私密 Obsidian Vault（独立窗口）
```

控制 Vault 与私密 Vault 必须是两个不同的 Obsidian Vault。若把 Bridge 装在私密 Vault 中，Vault 被锁定后 Bridge 也无法继续承担生命周期管理。
...
6. 插件将已挂载目录作为独立 Obsidian Vault 打开，并保存无密码配置；后续可直接使用“迁移此文件夹到私密笔记库”。

“创建私密笔记库”表示迁移到一个独立 Cryptomator Vault，不表示把原文件夹原地转换成密文目录。

## 日常使用

- 解锁：当前控制 Vault 已由 Nutstore 插件同步且密文 Vault 可访问时，输入一次密码；CLI 挂载后打开私密 Vault。
...
| 本机配置 | CLI 路径、控制 Vault 内的同步范围、挂载路径、已注册 Vault 名称 | 同步范围默认由当前 Vault 根目录自动填充；其他绝对路径每台电脑单独确认 |
| 敏感信息 | 密码、恢复密钥、完整原始诊断 | 不保存、不同步、不记录 |

挂载目录不能放在控制 Vault 或 Nutstore 插件同步范围内，防止 WebDAV 插件上传明文。私密 Obsidian Vault 中不得启用 Nutstore 插件。若密文路径不可访问，插件应提示检查控制 Vault 的同步状态，不能继续解锁。

## 范围外

- 不控制 Nutstore 插件或 WebDAV 服务，也不依赖其未公开 API；只根据密文 Vault 路径是否可访问来判断能否解锁。
