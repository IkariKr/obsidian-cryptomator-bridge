...
- `src/prerequisites.ts`：CLI 可执行性、`0.6.2` 版本门禁、挂载器列表发现（`discoverMounters`）、指定 mounter 在可用列表中、Vault 结构标记检查（`masterkey.cryptomator`/`vault.cryptomator`/`d`）。
...
| CLI 版本门禁 | PASS；`parseCliVersion` 与 `checkPrerequisites` 实现并覆盖 |
